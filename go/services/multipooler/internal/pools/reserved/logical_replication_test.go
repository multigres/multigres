...
func TestNewLogicalReplicationConnTagsReasonAndStartupParam(t *testing.T) {
	...
	defer conn.ReleaseDirty(DirtyReleaseError) // Pooled wrapper has pool=nil → closes the underlying socket.
...
func TestNewLogicalReplicationConnBlocksWhenCapFull(t *testing.T) {
	...
	c1.ReleaseDirty(DirtyReleaseError)
	...
	defer c4.ReleaseDirty(DirtyReleaseError)
	defer c2.ReleaseDirty(DirtyReleaseError)
}
...
func TestNewLogicalReplicationConnDialFailureFreesSlot(t *testing.T) {
	...
	require.NoError(t, err)
	ok.ReleaseDirty(DirtyReleaseError)
	...
	recovered.ReleaseDirty(DirtyReleaseError)
}
...
func TestPoolStatsLogicalReplicationActive(t *testing.T) {
	...
	c2.ReleaseDirty(DirtyReleaseError)
	plain.ReleaseClean(CleanReleaseCommit)
}
