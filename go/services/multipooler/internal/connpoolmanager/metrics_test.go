...
func TestMetrics_RegisterManagerCallbacks_RoutingRoleLabel(t *testing.T) {
	...
	for _, name := range []string{
		...
		requireDataPointWithAttrs(t, metric.Data, map[string]string{"routing_role": "primary"})
	}
...
func TestMetrics_RoutingRoleLabelValues(t *testing.T) {
	...
	require.NotPanics(t, func() {
		nilMetrics.SetRoutingRole(servingstate.RoutingRolePrimary)
	})
	require.Equal(t, "unknown", nilMetrics.routingRoleLabel())

	require.Equal(t, "unknown", m.routingRoleLabel())

	m.SetRoutingRole(servingstate.RoutingRolePrimary)
	require.Equal(t, "primary", m.routingRoleLabel())

	m.SetRoutingRole(servingstate.RoutingRoleReplica)
	require.Equal(t, "replica", m.routingRoleLabel())

	m.SetRoutingRole(servingstate.RoutingRoleUnknown)
	require.Equal(t, "unknown", m.routingRoleLabel())
}

func TestManager_OnStateChangeUpdatesMetricsRoutingRole(t *testing.T) {
	m, _ := setupPoolerMetrics(t)
	...
	require.NoError(t, nilManager.OnStateChange(
		context.Background(),
		servingstate.State{
			Routing:       servingstate.RoutingState{Role: servingstate.RoutingRolePrimary},
			ServingStatus: clustermetadatapb.PoolerServingStatus_SERVING,
		},
	))
}
