...
func TestManager_NewReservedConn(t *testing.T) {
	...
	conn.ReleaseClean(reserved.CleanReleaseCommit) // ReleaseCommit
}
...
func TestManager_NewReservedConn_SurvivesReopen(t *testing.T) {
	...
	require.NoError(t, err)
	c1.ReleaseClean(reserved.CleanReleaseCommit)
	...
	require.NoError(t, err)
	require.NotNil(t, c2)
	c2.ReleaseClean(reserved.CleanReleaseCommit)
}
...
func TestManager_ApplySettingsToConn(t *testing.T) {
	...
	require.NoError(t, err)
	defer conn.ReleaseClean(reserved.CleanReleaseCommit)
