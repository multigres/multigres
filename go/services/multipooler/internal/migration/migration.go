...
const MigrationNameUniqueIndexSQL = `CREATE UNIQUE INDEX IF NOT EXISTS migration_name_key
	ON multigres.migration (name) WHERE name IS NOT NULL`

// alterMigrationAddColumnsSQL forward-migrates a multigres.migration table that
// was created before name / copy_data / skip_schema_copy existed. Each ADD COLUMN
// IF NOT EXISTS is idempotent and a no-op on a table already carrying the column.
var alterMigrationAddColumnsSQL = []string{
