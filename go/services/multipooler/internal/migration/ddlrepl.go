...
//
//   - Publisher side: a shared multigres.ddl_log table plus two event triggers —
//     multigres.capture_ddl on ddl_command_end (ALTER TABLE) and
//     multigres.capture_drop on sql_drop (DROP TABLE) — that append the executing
//     statement to ddl_log. Because the INSERT commits in the SAME
...
//     none. The event triggers are REFCOUNTED via ddl_capture_tables:
//     tearing down one migration only drops them once no migration's
...
//     CREATE TABLE, CREATE INDEX (incl. CONCURRENTLY), DROP INDEX, and non-table
//     DDL — is not replicated, so those changes drift on the subscriber.
//   - CREATE TABLE is not captured at all. The migration's table set is fixed when
//     the migration starts, so a brand-new table is out of scope, and the listed
//     tables already exist (copied by the initial schema copy). Opting a new table
//     into a migration is out of scope (it would need a membership insert plus
//     adding the table to the publication).
...
CREATE TABLE IF NOT EXISTS multigres.ddl_apply (
	...
	// captureFunctionSQL captures ALTER TABLE, scoped to the migrated tables via the
	// ddl_capture_tables join. CREATE TABLE is intentionally NOT captured: the
	// migration's table set is fixed when the migration starts, so a brand-new
	// table is never in scope, and the listed tables already exist (copied by the
	// initial schema copy). DROP TABLE is captured separately on the sql_drop event
	// (capture_drop). CREATE INDEX and every other DDL kind are excluded. The one
	// non-transactional statement carrying the ALTER TABLE tag —
	...
	// distinguish it): it cannot run inside the apply worker's transaction. Must be
	// LANGUAGE plpgsql: a SQL function cannot return the event_trigger pseudo-type.
	captureFunctionSQL = `
...
$fn$`
	...
	// is scoped to the migrated tables via the ddl_capture_tables join and fans one
	// ddl_log row out per owning migration, storing current_query() (the DROP
	// statement) for replay. Only tables the user explicitly dropped are captured
	// (d.original) — cascade/internal drops (indexes, sequences, a partition's
	// children) are skipped; the replayed DROP handles its own cascades. Must be
	// LANGUAGE plpgsql (event_trigger return type).
	captureDropFunctionSQL = `
CREATE OR REPLACE FUNCTION multigres.capture_drop()
	RETURNS event_trigger LANGUAGE plpgsql AS $fn$
BEGIN
	INSERT INTO multigres.ddl_log (migration_id, schema_name, table_name, ddl_command)
	SELECT m.migration_id, d.schema_name, d.object_name, current_query()
...
$fn$`
	...
	dropDropCaptureTriggerSQL   = `DROP EVENT TRIGGER IF EXISTS mg_capture_drop`
	...
	// dropCaptureTriggerExistsSQL is the refcount-independent existence check for
	// the shared sql_drop trigger, armed once alongside mg_capture_ddl.
...
`
	...
	dropCaptureDropFunctionSQL = `DROP FUNCTION IF EXISTS multigres.capture_drop()`
...
func setupDDLCapture(ctx context.Context, c ddlConn, migrationID string, tables []string) error {
	for _, stmt := range []string{ddlSchemaSQL, ddlLogTableSQL, ddlCaptureTablesSQL, captureFunctionSQL, captureDropFunctionSQL} {
...
// armDDLCapture creates the two shared event triggers if they do not already
// exist: mg_capture_ddl (ddl_command_end, CREATE/ALTER TABLE) and mg_capture_drop
// (sql_drop, DROP TABLE). It is refcount-safe: for the second and later concurrent
// migrations the triggers already exist, so this is a no-op and capture is never
...
func armDDLCapture(ctx context.Context, c ddlConn) error {
	for _, trig := range []struct{ existsSQL, createSQL string }{
		...
		if err := c.exec(ctx, trig.createSQL); err != nil {
...
func teardownDDLCapture(ctx context.Context, c ddlConn, migrationID string) error {
	...
	if remaining == 0 {
		...
		// Disarm both event triggers before dropping ddl_capture_tables — capture_drop
		// reads that table, and sql_drop fires on the DROP TABLE below, so the trigger
		// must be gone first.
		for _, stmt := range []string{
			dropCaptureTriggerSQL, dropDropCaptureTriggerSQL,
			dropCaptureFunctionSQL, dropCaptureDropFunctionSQL, dropCaptureTablesSQL,
		} {
			if err := c.exec(ctx, stmt); err != nil {
