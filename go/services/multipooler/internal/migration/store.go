...
func (s *Store) Update(ctx context.Context, m *Migration) error {
	...
	if _, err := tx.QueryArgs(
		...
		sequence_margin=$4, last_error=$5, streaming_since=$6
...
func (s *Store) GetByRef(ctx context.Context, ref string) (*Migration, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	if s.cache == nil {
		if err := s.loadCacheLocked(ctx); err != nil {
...
func scanMigration(row *sqltypes.Row) (*Migration, error) {
	...
	err := executor.ScanRow(row, &m.ID, &phase, &m.Name, &m.SourceDSN, &m.TargetDatabase, &m.TargetShard, &m.SequenceMargin, &m.CopyData, &m.SkipSchemaCopy, &m.LastError, &m.CreatedAt, &streaming, &tablesJSON)
	if err != nil {
...
// name admits multiple unnamed migrations; a non-empty name is stored verbatim.
