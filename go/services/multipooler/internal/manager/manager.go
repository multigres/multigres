...
type MultiPoolerManager struct {
	...
	// Deprecated: use serviceReplicaID instead.
	serviceID        *clustermetadatapb.ID
	serviceReplicaID consensus.ReplicaID
...
func NewMultiPoolerManagerWithTimeout(logger *slog.Logger, multiPooler *clustermetadatapb.MultiPooler, config *Config, loadTimeout time.Duration) (*MultiPoolerManager, error) {
	...
	svcReplicaID, err := consensus.NewReplicaID(multiPooler.Id)
	if err != nil {
	...
	pm.consensusState = consensus.NewConsensusState(pm.multipooler.PoolerDir, pm.serviceID)
	...
	pm.rules = consensus.NewRuleStore(pm.logger, pm.qsc.InternalQueryService())
...
func (pm *MultiPoolerManager) restoreAndStartPostgres(ctx context.Context) error {
	...
	currentTerm, _ := pm.consensusState.GetInconsistentCurrentTermNumber()
