...
// SetRevocationInMemory sets the in-memory revocation without disk I/O.
// Used by tests to inject a specific state without file system access.
func (cs *ConsensusState) SetRevocationInMemory(revocation *clustermetadatapb.TermRevocation) {
	cs.mu.Lock()
