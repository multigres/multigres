...
func TestRecoveryEngine_MaintenanceLoop(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		require.Equal(t, int32(1), reloadCount.Load())
		...
		re.mu.Unlock()
		require.Equal(t, targets, expectedTargets)
	})
}
...
func TestRecoveryEngine_ViperDynamicConfig(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		synctest.Wait()
		re.mu.Lock()
		actual := slices.Clone(re.shardWatchTargets)
		re.mu.Unlock()
		require.Equal(t, actual, expectedFinal)
		re.Shutdown()
	})
}
