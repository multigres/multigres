		...
		if errors.Is(wd.Err, &topoclient.TopoError{Code: topoclient.NoNode}) {
			if strings.HasSuffix(wd.Path, "/"+topoclient.PoolerFile) {
				...
				if poolerID != "" {
					...
					// Do not remove from store here; bookkeeping handles stale entry removal.
					// This mirrors the existing polling approach where deleted poolers are
					// forgotten by runBookkeeping rather than by the discovery path.
				}
			}
		} else {
