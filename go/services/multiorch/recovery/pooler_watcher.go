	...
	if wd.Err != nil {
		...
		if errors.Is(wd.Err, &topoclient.TopoError{Code: topoclient.NoNode}) {
			if strings.HasSuffix(wd.Path, "/"+topoclient.PoolerFile) {
				...
				// We do not remove deleted poolers from the store here;
				// bookkeeping handles stale entry removal.
				//
				// This mirrors the previoius polling approach where deleted
				// poolers are forgotten by runBookkeeping rather than by the
				// discovery path.
				if poolerID != "" {
					cw.logger.Debug("pooler deleted from topology", "pooler_id", poolerID)
				}
			}
		} else {
