...
func TestBufferBasicBufferingAndDrain(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		retryDone, err := buf.WaitForFailoverEnd(ctx, "tg1", "shard1")
		waitErr = err
		if retryDone != nil {
			gotRetryDone = true
...
func TestBufferDisabled(t *testing.T) {
	...
	retryDone, err := buf.WaitForFailoverEnd(context.Background(), "tg1", "shard1")
...
func TestBufferTimingGuard(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		retryDone, waitErr := buf.WaitForFailoverEnd(ctx, "tg1", "shard1")
		assert.NoError(t, waitErr)
		if retryDone != nil {
			retryDone()
		}
...
func TestBufferMultipleShards(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		retryDone, err := buf.WaitForFailoverEnd(ctx, "tg1", "shard2")
		err2 = err
		if retryDone != nil {
			retryDone()
		}
	...
	buf.mu.Lock()
	assert.Len(t, buf.queue, 1)
	assert.Equal(t, "tg1/shard2", buf.queue[0].shardKey)
	buf.mu.Unlock()

	// Stop buffering for shard2.
	buf.StopBuffering("tg1", "shard2")
	wg.Wait()
...
func TestBufferDrainingSkipsNewRequests(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		var err error
		firstRetryDone, err = buf.WaitForFailoverEnd(ctx, "tg1", "shard1")
...
func TestBufferShutdownAfterEnqueue(t *testing.T) {
	...
	for range 5 {
		wg.Go(func() {
			_, _ = buf.WaitForFailoverEnd(ctx, "tg1", "shard1")
		})
	}
