...
func TranslateSQLPreparedStatementError(err error, logicalName, sourceSQL string, stmt *ast.ExecuteStmt) error {
	// Without a logical name there is nothing to translate to — rewriting
	// would turn a ppstmt* mention into `prepared statement ""`.
	if logicalName == "" {
		return err
	}
...
// PrepareBodyOffset returns the byte offset of the body in `PREPARE name
// [(types)] AS <body>`, or 0 if it cannot be located. The AST is no help here:
// the body node's Location() is 0, not its offset in the source.
func PrepareBodyOffset(sourceSQL string) int {
	if sourceSQL == "" {
		return 0
	}
	lexer := parser.NewLexer(sourceSQL)
	seenPrepare, seenName, depth := false, false, 0
	for token := lexer.NextToken(); token.Type != parser.EOF && token.Type != parser.INVALID; token = lexer.NextToken() {
		switch {
		case !seenPrepare:
			if token.Type != parser.PREPARE {
				return 0
			}
			seenPrepare = true
		case !seenName:
		...
		case token.Text == ")":
			depth--
		// Only the AS outside the parameter-type list separates name from body.
		case depth == 0 && token.Type == parser.AS:
			body := lexer.NextToken()
			if body.Type == parser.EOF || body.Type == parser.INVALID {
				return 0
			}
