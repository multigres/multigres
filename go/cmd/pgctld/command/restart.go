...
func (s *PgCtldService) RestartPostgreSQLWithResult(mode string, asStandby bool) (*RestartResult, error) {
	...
	if isPostgreSQLRunning(config.PostgresDataDir) {
		...
		logger.Info("PostgreSQL is not running, proceeding with start") //nolint:sloglint // message intentionally starts with an operation name or proper noun
	...
	if asStandby {
		...
		logger.Info("PostgreSQL server restarted as standby successfully", "pid", result.PID) //nolint:sloglint // message intentionally starts with an operation name or proper noun
	} else {
		result.Message = "PostgreSQL server restarted successfully"
		logger.Info("PostgreSQL server restarted successfully") //nolint:sloglint // message intentionally starts with an operation name or proper noun
	}
