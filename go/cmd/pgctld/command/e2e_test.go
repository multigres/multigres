...
`), 0644)
	...
	t.Run("startup_performance", func(t *testing.T) {
		...
		startCmd := exec.Command(pgctldBinary, "start", "--pg-data-dir", dataDir, "--pg-port", strconv.Itoa(perfTestPort), "--config", pgctldConfigFile)
	...
	t.Run("multiple_rapid_operations", func(t *testing.T) {
		...
		for i := range 3 {
			...
			// Start
			startCmd := exec.Command(pgctldBinary, "start", "--pg-data-dir", dataDir, "--pg-port", strconv.Itoa(rapidTestPort), "--config", pgctldConfigFile)
			setupTestEnv(startCmd)
			err := startCmd.Run()
			require.NoError(t, err)
...
`), 0644)
	...
	t.Run("version_compatibility", func(t *testing.T) {
		...
		startCmd := exec.Command(pgctldBinary, "start", "--pg-data-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config", pgctldConfigFile)
		setupTestEnv(startCmd)
		...
		statusCmd := exec.Command(pgctldBinary, "status", "--pg-data-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config", pgctldConfigFile)
		setupTestEnv(statusCmd)
		output, err = statusCmd.Output()
		require.NoError(t, err)
	...
	t.Run("configuration_management", func(t *testing.T) {
		...
		startCmd := exec.Command(pgctldBinary, "start", "--pg-data-dir", configDataDir, "--pg-port", strconv.Itoa(configTestPort), "--config", pgctldConfigFile)
		setupTestEnv(startCmd)
		startOutput, err := startCmd.CombinedOutput()
		if err != nil {
			t.Logf("pgctld start failed with output: %s", string(startOutput))
		...
		statusCheckCmd := exec.Command(pgctldBinary, "status", "--pg-data-dir", configDataDir, "--pg-port", strconv.Itoa(configTestPort), "--config", pgctldConfigFile)
...
`
		...
		reloadCmd := exec.Command(pgctldBinary, "reload-config", "--pg-data-dir", configDataDir, "--config", pgctldConfigFile)
		...
		statusCmd := exec.Command(pgctldBinary, "status", "--pg-data-dir", configDataDir, "--pg-port", strconv.Itoa(configTestPort), "--config", pgctldConfigFile)
		setupTestEnv(statusCmd)
		output, err := statusCmd.Output()
		require.NoError(t, err)
		...
		// Clean shutdown
		stopCmd := exec.Command(pgctldBinary, "stop", "--pg-data-dir", configDataDir, "--config", pgctldConfigFile)
		setupTestEnv(stopCmd)
		err = stopCmd.Run()
		require.NoError(t, err)
	})
