...
func checkFileInCell(t *testing.T, conn topo.Conn, hasCells bool) {
	...
	if _, err = conn.Update(ctx, "/myfile", []byte{'b'}, version); !errors.Is(err, &topo.TopoError{Code: topo.BadVersion}) {
	...
	if err = conn.Delete(ctx, "/myfile", version); !errors.Is(err, &topo.TopoError{Code: topo.BadVersion}) {
	...
	if err = conn.Delete(ctx, "/myfile", newVersion); !errors.Is(err, &topo.TopoError{Code: topo.NoNode}) {
