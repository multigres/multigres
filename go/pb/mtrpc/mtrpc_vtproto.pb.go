...
func (m *RPCError) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.PgDiagnostic != nil {
		size, err := m.PgDiagnostic.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *RPCError) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PgDiagnostic", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.PgDiagnostic == nil {
				m.PgDiagnostic = &query.PgDiagnostic{}
			}
			if err := m.PgDiagnostic.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		default:
