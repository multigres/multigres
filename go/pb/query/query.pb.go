...
type ExecuteOptions struct {
	...
	// eager_parse_prepared_statement, if set, asks the multipooler to Parse this
	// prepared statement as the unnamed statement on the reserved backend without
	// executing it. This preserves PostgreSQL's transaction-time validation and
	// lock-acquisition semantics for a PREPARE/Parse issued inside an explicit
	// transaction: the backend is pinned for the transaction, so an unnamed Parse
	// there is sufficient. Presence (non-nil) is the request; there is no flag.
	//
	// Only consumed by StreamExecute. PortalStreamExecute has its own top-level
	// prepared_statement field.
	EagerParsePreparedStatement *PreparedStatement `protobuf:"bytes,7,opt,name=eager_parse_prepared_statement,json=eagerParsePreparedStatement,proto3" json:"eager_parse_prepared_statement,omitempty"`
...
func (x *ExecuteOptions) GetEagerParsePreparedStatement() *PreparedStatement {
	if x != nil {
		return x.EagerParsePreparedStatement
	}
	return nil
}
...
const file_query_proto_rawDesc = "" +
	...
	"\x12backend_process_id\x18\x04 \x01(\rR\x10backendProcessId\"\xf8\x03\n" +
	...
	"\x14client_connection_id\x18\x06 \x01(\rR\x12clientConnectionId\x12]\n" +
	"\x1eeager_parse_prepared_statement\x18\a \x01(\v2\x18.query.PreparedStatementR\x1beagerParsePreparedStatement\x12'\n" +
...
var file_query_proto_goTypes = []any{
	...
	nil,                              // 16: query.QueryResult.ParameterStatusEntry
...
var file_query_proto_depIdxs = []int32{
	...
	16, // 6: query.QueryResult.parameter_status:type_name -> query.QueryResult.ParameterStatusEntry
	...
	18, // 9: query.Target.shard_key:type_name -> clustermetadata.ShardKey
	...
	14, // 13: query.ExecuteOptions.user_auth:type_name -> query.UserAuth
	10, // 14: query.ExecuteOptions.eager_parse_prepared_statement:type_name -> query.PreparedStatement
