...
`
			...
			err := exec.CommandContext(t.Context(), "sh", "-c", "cat > "+pgbackrestPath+" << 'EOF'\n"+mockScript+"\nEOF").Run()
			require.NoError(t, err)
			err = exec.CommandContext(t.Context(), "chmod", "+x", pgbackrestPath).Run()
			require.NoError(t, err)
...
func TestSafeCombinedOutput(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cmd := exec.CommandContext(t.Context(), tt.command, tt.args...)
...
func TestSafeCombinedOutput_LargeOutput(t *testing.T) {
	...
	t.Run("Large output exceeding channel buffer", func(t *testing.T) {
		cmd := exec.CommandContext(t.Context(), "sh", "-c", "for i in $(seq 1 200); do echo \"Line $i\"; done")
		output, err := safeCombinedOutput(cmd)
	...
	t.Run("Very large output (1000 lines)", func(t *testing.T) {
		cmd := exec.CommandContext(t.Context(), "sh", "-c", "for i in $(seq 1 1000); do echo \"Line $i\"; done")
		output, err := safeCombinedOutput(cmd)
...
func TestSafeCombinedOutput_LongLines(t *testing.T) {
	...
	t.Run("Very long single line", func(t *testing.T) {
		...
		cmd := exec.CommandContext(t.Context(), "echo", longString)
		output, err := safeCombinedOutput(cmd)
...
func TestSafeCombinedOutput_RapidOutput(t *testing.T) {
	...
	t.Run("Rapid burst of output", func(t *testing.T) {
		...
		cmd := exec.CommandContext(t.Context(), "sh", "-c", "yes 'rapid output line' | head -n 500")
		output, err := safeCombinedOutput(cmd)
...
func TestSafeCombinedOutput_EmptyStreams(t *testing.T) {
	t.Run("Only stdout", func(t *testing.T) {
		cmd := exec.CommandContext(t.Context(), "echo", "only stdout")
		output, err := safeCombinedOutput(cmd)
	...
	t.Run("Only stderr", func(t *testing.T) {
		cmd := exec.CommandContext(t.Context(), "sh", "-c", "echo 'only stderr' >&2")
		output, err := safeCombinedOutput(cmd)
...
func TestSafeCombinedOutput_BinaryOutput(t *testing.T) {
	...
	t.Run("Binary-like output", func(t *testing.T) {
		...
		cmd := exec.CommandContext(t.Context(), "sh", "-c", "printf 'text\\x00with\\x00nulls\\n'")
		output, err := safeCombinedOutput(cmd)
...
func TestSafeCombinedOutput_CommandNotFound(t *testing.T) {
	t.Run("Command does not exist", func(t *testing.T) {
		cmd := exec.CommandContext(t.Context(), "this-command-definitely-does-not-exist-12345")
...
func BenchmarkSafeCombinedOutput(b *testing.B) {
	b.Run("Small output", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := exec.CommandContext(b.Context(), "echo", "hello world")
	...
	b.Run("Medium output (100 lines)", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := exec.CommandContext(b.Context(), "sh", "-c", "for i in $(seq 1 100); do echo \"Line $i\"; done")
			_, _ = safeCombinedOutput(cmd)
		}
	})
	...
	b.Run("Large output (1000 lines)", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			cmd := exec.CommandContext(b.Context(), "sh", "-c", "for i in $(seq 1 1000); do echo \"Line $i\"; done")
			_, _ = safeCombinedOutput(cmd)
		}
	})
	...
	b.Run("Mixed stdout and stderr", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			cmd := exec.CommandContext(b.Context(), "sh", "-c", script)
			_, _ = safeCombinedOutput(cmd)
		}
	})
}
