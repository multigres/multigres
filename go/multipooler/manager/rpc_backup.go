...
// checkPgBackRestConfig returns the path to the pgbackrest.conf generated by pgctld.
// The config file is expected to already exist, created by pgctld during startup.
// Retries up to 5 times with exponential backoff if the file is not found.
func (pm *MultiPoolerManager) checkPgBackRestConfig(ctx context.Context) (string, error) {
	pgbackrestPath := pm.pgbackrestPath()
	configPath := filepath.Join(pgbackrestPath, "pgbackrest.conf")

	r := retry.New(100*time.Millisecond, 10*time.Second)

	for attempt, err := range r.Attempts(ctx) {
		if err != nil {
			return "", fmt.Errorf("context cancelled while waiting for pgbackrest config at %s: %w", configPath, err)
		}
		...
		// Stop after 5 retries (attempts 0-5 = 6 total attempts)
		if attempt >= 5 {
			return "", fmt.Errorf("pgbackrest config not found at %s after %d attempts (should be generated by pgctld)", configPath, attempt+1)
		}

		pm.logger.WarnContext(ctx, "pgBackRest config not found, retrying",
			"path", configPath,
			"attempt", attempt+1)
	}

	return "", fmt.Errorf("pgbackrest config not found at %s (should be generated by pgctld)", configPath)
}
