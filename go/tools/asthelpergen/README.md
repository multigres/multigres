...
The AST Helper Generator analyzes the AST node types in `go/parser/ast/` and automatically generates:

1. **Rewrite Methods** (`ast_rewrite.go`): Tree-rewriting functionality using the visitor pattern
...
With 200+ AST node types in Multigres, manually writing and maintaining Rewrite and Clone methods would be:
...
- **Code Generation**: `github.com/dave/jennifer/jen` to generate clean, idiomatic Go code
