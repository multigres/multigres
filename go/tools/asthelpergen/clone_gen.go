...
type cloneGen struct {
	exclude []string
	// pkgPath is the import path of the package being generated (the root
	// interface's package). Fields whose type is defined in a different package
	// are "foreign" — they belong to another AST hierarchy, so we shallow-copy
	// them (the leading `out := *n` carries them along) rather than trying to
	// deep-clone a type whose implementations we cannot see. See typePackagePath.
	pkgPath string
	file    *jen.File
}
...
func newCloneGen(pkgname, pkgPath string, options *CloneOptions) *cloneGen {
...
func (c *cloneGen) ptrToStructMethod(t types.Type, strct *types.Struct, spi generatorSPI) error {
	...
	for field := range strct.Fields() {
		...
		// Skip foreign-package fields: they belong to another AST hierarchy
		// (e.g. plpgsqlast.PLpgSQL_expr.Parsed is an ast.Stmt). We cannot deep
		// clone a type whose implementations are outside the package being
		// generated, and we shouldn't — the other hierarchy owns its own
		// cloning. The leading `out := *n` already shallow-copies the field, so
		// the clone shares the reference, which is the right boundary behavior.
		if fp := typePackagePath(field.Type()); fp != "" && fp != c.pkgPath {
			continue
		}
...
// typePackagePath returns the import path of the package that defines t,
// unwrapping pointers and slices to reach the underlying named type. It returns
// "" for types with no owning package (basic types, or slices/pointers of them).
func typePackagePath(t types.Type) string {
	switch u := t.(type) {
	case *types.Pointer:
		return typePackagePath(u.Elem())
	case *types.Slice:
		return typePackagePath(u.Elem())
	case *types.Named:
