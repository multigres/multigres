...
// revisionOverride and commitTimeOverride are optional link-time
// overrides for the VCS metadata that the Go toolchain normally embeds
// through runtime/debug.BuildInfo. They exist for builds that cannot
// read VCS info from BuildInfo.
// When set via -ldflags -X they take precedence over BuildInfo so the
// in-image --version output matches the OCI image labels.
// `commitTimeOverride` must be an RFC3339 timestamp, an unparsable value
// is ignored and falls back to BuildInfo.
...
func loadSnapshot() {
	...
	// Link-time overrides win when present (Docker builds without .git).
	if revisionOverride != "" {
		snapshotData.Revision = revisionOverride
	}
	if commitTimeOverride != "" {
		if t, err := time.Parse(time.RFC3339, commitTimeOverride); err == nil {
			snapshotData.CommitTime = t
		}
	}
}
