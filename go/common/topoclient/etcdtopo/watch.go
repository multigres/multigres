			...
			case wresp, ok := <-watcher:
				...
				for _, ev := range wresp.Events {
					...
					case mvccpb.DELETE:
						...
						notifications <- &topoclient.WatchData{
							Err: topoclient.NewError(topoclient.NoNode, nodePath),
						}
						return
					default:
						notifications <- &topoclient.WatchData{
...
func (s *Server) WatchRecursive(ctx context.Context, dirpath string) ([]*topoclient.WatchDataRecursive, <-chan *topoclient.WatchDataRecursive, error) {
	...
	for _, kv := range initial.Kvs {
		var wd topoclient.WatchDataRecursive
	...
	go func() {
		...
		for {
			...
			case <-watchCtx.Done():
				// This includes context cancellation errors.
				notifications <- &topoclient.WatchDataRecursive{
					WatchData: topoclient.WatchData{Err: convertError(watchCtx.Err(), nodePath)},
				}
				return
			case wresp, ok := <-watcher:
				if !ok {
				...
				if wresp.Canceled {
					// Final notification.
					notifications <- &topoclient.WatchDataRecursive{
						WatchData: topoclient.WatchData{Err: convertError(wresp.Err(), nodePath)},
					}
					return
				}
