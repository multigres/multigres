...
func (c *conn) Create(ctx context.Context, filePath string, contents []byte) (topoclient.Version, error) {
	...
	if _, ok := p.children[file]; ok {
		return nil, topoclient.NewError(topoclient.NodeExists, file)
	}
	...
	n.propagateRecursiveWatch(&topoclient.WatchDataRecursive{
		Path: filePath,
...
func (c *conn) Update(ctx context.Context, filePath string, contents []byte, version topoclient.Version) (topoclient.Version, error) {
	...
	if p == nil {
		...
		if version != nil {
			return nil, topoclient.NewError(topoclient.NoNode, filePath)
		}
		p = c.factory.getOrCreatePath(c.cell, dir)
		if p == nil {
	...
	if version != nil && n.version != uint64(version.(NodeVersion)) {
		return nil, topoclient.NewError(topoclient.BadVersion, filePath)
	}
	...
	for _, w := range n.watches {
		if w.contents != nil {
			w.contents <- &topoclient.WatchData{
				Contents: n.contents,
				Version:  NodeVersion(n.version),
			}
		}
...
func (c *conn) GetVersion(ctx context.Context, filePath string, version int64) ([]byte, error) {
	return nil, topoclient.NewError(topoclient.NoImplementation, "GetVersion not supported in memory topo")
}
...
func (c *conn) List(ctx context.Context, filePathPrefix string) ([]topoclient.KVInfo, error) {
	...
	if n == nil {
		return []topoclient.KVInfo{}, topoclient.NewError(topoclient.NoNode, filePathPrefix)
	}

	var result []topoclient.KVInfo
	for name, child := range n.children {
		...
		if child.isDirectory() {
			...
			result = append(result, topoclient.KVInfo{
...
func (c *conn) Delete(ctx context.Context, filePath string, version topoclient.Version) error {
	...
	p := c.factory.nodeByPath(c.cell, dir)
	if p == nil {
		return topoclient.NewError(topoclient.NoNode, filePath)
	}
	...
	// Check the version.
	if version != nil && n.version != uint64(version.(NodeVersion)) {
		return topoclient.NewError(topoclient.BadVersion, filePath)
	}
	...
	for _, w := range n.watches {
		if w.contents != nil {
			w.contents <- &topoclient.WatchData{
				Err: topoclient.NewError(topoclient.NoNode, filePath),
			}
			close(w.contents)
		}
		if w.lock != nil {
