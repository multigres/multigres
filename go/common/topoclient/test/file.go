...
func checkFileInCell(t *testing.T, conn topoclient.Conn, hasCells bool) {
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.NoNode}), "Get(non-existent) didn't return ErrNoNode but: %v", err)
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.BadVersion}), "Update(bad version) didn't return ErrBadVersion but: %v", err)
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.BadVersion}), "Delete('/myfile', wrong version) returned bad error: %v", err)
	...
	err = conn.Delete(ctx, "/myfile", newVersion)
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.NoNode}), "Delete(already gone) returned bad error: %v", err)
