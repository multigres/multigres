...
func checkLock(t *testing.T, ctx context.Context, ts topoclient.Store) {
	...
	conn, err := ts.ConnForCell(t.Context(), topoclient.GlobalCell)
...
func checkLockTimeout(ctx context.Context, t *testing.T, conn topoclient.Conn) {
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.Timeout}), "Lock(again) should return Timeout error, got: %v", err)
	cancel()
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.Interrupted}), "Lock(interrupted) should return Interrupted error, got: %v", err)
...
func checkLockName(t *testing.T, ctx context.Context, ts topoclient.Store) {
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.Timeout}), "LockName(again) should return Timeout error, got: %v", err)
	cancel()
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.Interrupted}), "LockName(interrupted) should return Interrupted error, got: %v", err)
...
func checkLockNameWithTTL(t *testing.T, ctx context.Context, ts topoclient.Store) {
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.Timeout}), "LockNameWithTTL(again) should return Timeout error, got: %v", err)
	cancel()
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.Interrupted}), "LockNameWithTTL(interrupted) should return Interrupted error, got: %v", err)
...
func checkTryLockName(t *testing.T, ctx context.Context, ts topoclient.Store) {
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.NodeExists}), "TryLockName(again) should return NodeExists error, got: %v", err)
...
func checkTryLockTimeout(ctx context.Context, t *testing.T, conn topoclient.Conn) {
	...
	assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.NodeExists}), "TryLock should return NodeExists error, got: %v", err)
	cancel()
...
func checkTryLockUnblocks(ctx context.Context, t *testing.T, conn topoclient.Conn) {
	cellPath := path.Join(topoclient.DatabasesPath, "test_database")
	unblock := make(chan struct{})
	finished := make(chan struct{})
	...
	go func() {
		...
		for time.Now().Before(waitUntil) {
			...
			if err != nil {
				assert.True(t, errors.Is(err, &topoclient.TopoError{Code: topoclient.NodeExists}), "expected node exists during trylock, got: %v", err)
				time.Sleep(1 * time.Second)
			} else {
				if err = lockDescriptor.Unlock(ctx); err != nil {
					require.NoError(t, err, "Unlock(test_database) failed")
