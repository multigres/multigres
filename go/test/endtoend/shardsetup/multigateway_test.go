...
// TestMultigatewaySimpleProtocolPreparedStatements tests PREPARE/EXECUTE/DEALLOCATE
// via the simple query protocol, verifying that the gateway handles them locally
// using the prepared statement consolidator (analogous to the extended query protocol).
func TestMultigatewaySimpleProtocolPreparedStatements(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping multigateway test in short mode")
	}
	if utils.ShouldSkipRealPostgres() {
		t.Skip("PostgreSQL binaries not found, skipping")
	}
