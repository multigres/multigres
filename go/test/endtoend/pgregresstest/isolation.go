...
// whole transaction). The table is also created here, idempotently and
// ahead of the suite, so the shim never races the pooler's own lazy
// creation. A given vpid can map to multiple live PG backends (a leftover
// row for a pooled conn used by an earlier query, plus the live reserved
// conn) so the wait-check aggregates over every matching backend rather
// than picking one non-deterministically; rows of dead backends are
// ignored via the join against pg_stat_activity.
...
BEGIN
    ...
    -- the duplicate-mapping case where one backend is the live reserved
    -- conn (potentially blocked) and another is a leftover pool conn
    -- (idle).
    --
