...
// incident (see project_multigres_leader_health_bug memory): a recruit round
// selects a new leader, but its Promote RPC is lost (network partition,
// timeout, ...) while the SetPrimary RPCs to the other cohort members still
// land. rule_change.go's dispatchPromote sends both as independent,
// concurrent RPCs — Promote to the selected leader, SetPrimary to every
// other cohort member — so this is a real, reachable partial-failure race in
// the existing dispatch, not a fabricated state.
//
...
// would, and as the confirmed root cause's own analysis depends on: a
// pooler's TermRevocation must already be durably accepted before its
// SetPrimary/Promote is meaningful) — only the leader's Promote is skipped.
//
...
//
// Currently FAILS: a confirmed bug in leader_needs_replacement_analyzer.go
// means leaderPostgresReady only checks pgctld/DB liveness, never
// PostgresStatus == PRIMARY, so a live standby mistakenly recorded as leader
// passes the analyzer's happy path and no failover problem is ever
// (re-)detected — even though no pooler is actually primary. This test
// asserts the correct behavior (ordinary automatic recovery, matching the
// sibling tests in stuck_proposal_test.go) and should start passing once
// that's fixed.
