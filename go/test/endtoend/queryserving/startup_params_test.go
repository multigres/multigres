	...
	for _, target := range setup.GetComparisonTargets(t) {
		t.Run(target.Name, func(t *testing.T) {
			...
			t.Run("connection pooling isolation", func(t *testing.T) {
				...
				connStr1 := shardsetup.GetTestUserDSN("localhost", target.Port, "sslmode=disable")
				...
				connStr2 := shardsetup.GetTestUserDSN("localhost", target.Port, "sslmode=disable")
			...
			t.Run("server_version unchanged", func(t *testing.T) {
				...
				baselineConnStr := shardsetup.GetTestUserDSN("localhost", target.Port, "sslmode=disable")
			...
			t.Run("invalid startup parameter allows connection but fails queries", func(t *testing.T) {
				if target.Name != "multigateway" {
					t.Skip("invalid startup parameter handling is multigateway-specific")
				}
	...
	for _, target := range setup.GetComparisonTargets(t) {
		t.Run(target.Name, func(t *testing.T) {
			...
			t.Run("multiple -c flags", func(t *testing.T) {
				connStr := shardsetup.GetTestUserDSN("localhost", target.Port, "sslmode=disable", "connect_timeout=5", "options='-c work_mem=32MB -c lock_timeout=5s'")
			...
			t.Run("--key=value with hyphen-to-underscore", func(t *testing.T) {
				connStr := shardsetup.GetTestUserDSN("localhost", target.Port, "sslmode=disable", "connect_timeout=5", "options='--statement-timeout=10s'")
				...
				require.NoError(t, err)
				assert.Equal(t, "10s", result, "statement_timeout should be 10s (gateway-managed, PG display format)")
			})

			t.Run("mixed -c and -- formats", func(t *testing.T) {
				connStr := shardsetup.GetTestUserDSN("localhost", target.Port, "sslmode=disable", "connect_timeout=5", "options='-c work_mem=48MB --lock-timeout=3s'")
