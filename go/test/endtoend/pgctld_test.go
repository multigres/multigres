...
`), 0o644)
	...
	t.Run("basic_commands_with_real_postgresql", func(t *testing.T) {
		...
		initCmd := exec.CommandContext(t.Context(), "pgctld", "init", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		...
		controldataCmd := exec.CommandContext(t.Context(), "pg_controldata", pgDataDir)
		...
		statusCmd := exec.CommandContext(t.Context(), "pgctld", "status", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		...
		helpCmd := exec.CommandContext(t.Context(), "pgctld", "--help")
		...
		versionCmd := exec.CommandContext(t.Context(), "postgres", "--version")
		...
		initdbCmd := exec.CommandContext(t.Context(), "initdb", "--help")
...
`), 0o644)
	...
	t.Run("grpc_server_with_real_postgresql", func(t *testing.T) {
		...
		serverCmd := exec.CommandContext(t.Context(), "pgctld", "server",
			"--pooler-dir", dataDir,
...
`), 0o644)
	...
	t.Run("startup_performance", func(t *testing.T) {
		...
		startTime := time.Now()
		initCmd := exec.CommandContext(t.Context(), "pgctld", "init", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(perfTestPort), "--config-file", pgctldConfigFile)
		setupTestEnv(initCmd)
		...
		startCmd := exec.CommandContext(t.Context(), "pgctld", "start", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(perfTestPort), "--config-file", pgctldConfigFile)
		...
		// Clean shutdown
		stopCmd := exec.CommandContext(t.Context(), "pgctld", "stop", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(perfTestPort), "--config-file", pgctldConfigFile)
	...
	t.Run("multiple_rapid_operations", func(t *testing.T) {
		...
		for i := range 3 {
			...
			// Stop
			stopCmd := exec.CommandContext(t.Context(), "pgctld", "stop", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(perfTestPort), "--mode", "fast", "--config-file", pgctldConfigFile)
			setupTestEnv(stopCmd)
			err = stopCmd.Run()
			require.NoError(t, err)
...
`), 0o644)
	...
	t.Run("version_compatibility", func(t *testing.T) {
		...
		initCmd := exec.CommandContext(t.Context(), "pgctld", "init", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile, "--pg-port", strconv.Itoa(testPort))
		setupTestEnv(initCmd)
		initOutput, err := initCmd.CombinedOutput()
		if err != nil {
			t.Logf("pgctld init failed with output: %s", string(initOutput))
		...
		startCmd := exec.CommandContext(t.Context(), "pgctld", "start", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config-file", pgctldConfigFile)
		setupTestEnv(startCmd)
		startOutput, err := startCmd.CombinedOutput()
		if err != nil {
			t.Logf("pgctld start failed with output: %s", string(startOutput))
		...
		statusCmd := exec.CommandContext(t.Context(), "pgctld", "status", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config-file", pgctldConfigFile)
		setupTestEnv(statusCmd)
		...
		// Clean shutdown
		stopCmd := exec.CommandContext(t.Context(), "pgctld", "stop", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config-file", pgctldConfigFile)
		setupTestEnv(stopCmd)
		err = stopCmd.Run()
		require.NoError(t, err)
	})
...
`), 0o644)
	...
	// Initialize database
	initCmd := exec.CommandContext(t.Context(), "pgctld", "init", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--config-file", pgctldConfigFile)
	setupTestEnv(initCmd)
	initOutput, err := initCmd.CombinedOutput()
	if err != nil {
		t.Logf("pgctld init failed with output: %s", string(initOutput))
	...
	// Restart as standby
	restartCmd := exec.CommandContext(t.Context(), "pgctld", "restart", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(testPort), "--as-standby", "--config-file", pgctldConfigFile)
	...
	recoveryCheckCmd := exec.CommandContext(t.Context(), "psql",
		"-h", socketDir,
...
func TestPostgreSQLAuthentication(t *testing.T) {
	...
	t.Run("pgpassword_authentication", func(t *testing.T) {
		...
		initCmd := exec.CommandContext(t.Context(), "pgctld", "init", "--pooler-dir", baseDir, "--pg-port", strconv.Itoa(port))
		...
		startCmd := exec.CommandContext(t.Context(), "pgctld", "start", "--pooler-dir", baseDir, "--pg-port", strconv.Itoa(port))
		...
		socketCmd := exec.CommandContext(t.Context(), "psql",
			"-h", socketDir,
		...
		statusCmd := exec.CommandContext(t.Context(), "pgctld", "status", "--pooler-dir", baseDir)
		...
		tcpCmd := exec.CommandContext(t.Context(), "psql",
			"-h", "localhost",
		...
		wrongPasswordCmd := exec.CommandContext(t.Context(), "psql",
			"-h", "localhost",
			"-p", strconv.Itoa(port),
			"-U", "postgres",
			"-d", "postgres",
		...
		roleCheckCmd := exec.CommandContext(t.Context(), "psql",
			"-h", socketDir,
			"-p", strconv.Itoa(port),
			"-U", "postgres",
			"-d", "postgres",
		...
		dbCheckCmd := exec.CommandContext(t.Context(), "psql",
			"-h", socketDir,
			"-p", strconv.Itoa(port),
			"-U", "postgres",
			"-d", "postgres",
		...
		privilegeCheckCmd := exec.CommandContext(t.Context(), "psql",
			"-h", socketDir,
			"-p", strconv.Itoa(port),
			"-U", "postgres",
			"-d", "postgres",
		...
		stopCmd := exec.CommandContext(t.Context(), "pgctld", "stop", "--pooler-dir", baseDir)
	...
	t.Run("password_file_authentication", func(t *testing.T) {
		...
		initCmd := exec.CommandContext(t.Context(), "pgctld", "init", "--pooler-dir", baseDir, "--pg-pwfile", pwfile, "--pg-port", strconv.Itoa(port))
	...
	t.Run("password_source_conflict", func(t *testing.T) {
		...
		initCmd := exec.CommandContext(t.Context(), "pgctld", "init", "--pooler-dir", baseDir, "--pg-pwfile", pwfile)
		initCmd.Env = append(os.Environ(),
			"PGCONNECT_TIMEOUT=5",
...
`), 0o644)
	...
	t.Run("complete_lifecycle_via_cli", func(t *testing.T) {
		...
		startCmd := exec.CommandContext(t.Context(), "pgctld", "start", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		startCmd.Env = append(os.Environ(),
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		err = startCmd.Run()
		require.NoError(t, err)
		...
		reloadCmd := exec.CommandContext(t.Context(), "pgctld", "reload-config", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		...
		restartCmd := exec.CommandContext(t.Context(), "pgctld", "restart", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		...
		stopCmd := exec.CommandContext(t.Context(), "pgctld", "stop", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
...
`), 0o644)
	...
	for i := range 3 {
		t.Run(fmt.Sprintf("cycle_%d", i+1), func(t *testing.T) {
			...
			// Stop PostgreSQL
			stopCmd := exec.CommandContext(t.Context(), "pgctld", "stop", "--pooler-dir", dataDir, "--mode", "fast", "--config-file", pgctldConfigFile)
			stopCmd.Env = append(os.Environ(),
				"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
			err = stopCmd.Run()
			require.NoError(t, err)
...
`), 0o644)
	...
	t.Run("start_with_nonexistent_data_dir", func(t *testing.T) {
		...
		startCmd := exec.CommandContext(t.Context(), "pgctld", "start", "--pooler-dir", nonexistentDir, "--config-file", pgctldConfigFile)
		startCmd.Env = append(os.Environ(),
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		err := startCmd.Run()
		...
		initCmd := exec.CommandContext(t.Context(), "pgctld", "init", "--pooler-dir", nonexistentDir, "--config-file", pgctldConfigFile)
		initCmd.Env = append(os.Environ(),
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		err = initCmd.Run()
		require.NoError(t, err)
		...
		// Clean stop
		stopCmd := exec.CommandContext(t.Context(), "pgctld", "stop", "--pooler-dir", nonexistentDir, "--mode", "immediate", "--config-file", pgctldConfigFile)
		stopCmd.Env = append(os.Environ(),
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		_ = stopCmd.Run()
	})
	...
	t.Run("double_start_attempt", func(t *testing.T) {
		...
		startCmd2 := exec.CommandContext(t.Context(), "pgctld", "start", "--pooler-dir", dataDir, "--config-file", pgctldConfigFile)
		...
		// Clean stop
		stopCmd := exec.CommandContext(t.Context(), "pgctld", "stop", "--pooler-dir", dataDir, "--mode", "immediate", "--config-file", pgctldConfigFile)
		stopCmd.Env = append(os.Environ(),
			"PATH="+filepath.Join(tempDir, "bin")+":"+os.Getenv("PATH"))
		_ = stopCmd.Run()
	})
...
func TestOrphanDetectionWithRealPostgreSQL(t *testing.T) {
	...
	initCmd := exec.CommandContext(t.Context(), "pgctld", "init", "--pooler-dir", dataDir, "--pg-port", strconv.Itoa(pgPort), "--config-file", pgctldConfigFile)
	setupTestEnv(initCmd)
