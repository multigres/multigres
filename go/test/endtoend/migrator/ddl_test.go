...
// TestDDLReplicationDropTable exercises DROP TABLE replication (capture_drop on
// the sql_drop event): a migrated table dropped on the source is dropped on the
// target, while an unrelated (non-migrated) table dropped on the source is NOT.
// DROP is invisible to ddl_command_end/pg_event_trigger_ddl_commands(), so it
// rides a separate sql_drop event trigger.
func TestDDLReplicationDropTable(t *testing.T) {
	if testing.Short() {
		t.Skip("skipping Multigres Migrator e2e in short mode")
	}
	if !utils.HasPostgreSQLBinaries() {
	...
	// A second migrated table (dropped later) and an unrelated table (must NOT be
	// dropped on the target). Both created before StartMigration so the migrated
	// one is schema-copied and published.
	sc := dialSource(t, ctx, srcPort)
	defer sc.Close()
	for _, stmt := range []string{
		"CREATE TABLE public.extra (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, v text)",
		"INSERT INTO public.extra (v) VALUES ('x'), ('y')",
		"CREATE TABLE public.unrelated (id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY, v text)",
	} {
		_, err := sc.Query(ctx, stmt)
		require.NoError(t, err, stmt)
	}
	...
	createResp, err := mt.CreateMigration(ctx, &migratorpb.CreateMigrationRequest{
		SourceDsn:      sourceDSN(srcPort),
		TargetDatabase: targetDB,
		Objects:        objs("public.orders", "public.extra"),
	})
	require.NoError(t, err)
	id := createResp.GetMigration().GetId()
	...
	tc := targetConn(t, ctx, primary, targetDB)
	defer tc.Close()
	// Both migrated tables land on the target.
	require.Eventually(t, func() bool {
		n, ok := countRows(t, ctx, tc, "public.extra")
		return ok && n == 2
	}, 30*time.Second, 500*time.Millisecond, "extra must copy to target")
	require.Equal(t, "id,v", columnsOf(t, ctx, tc, "public", "extra"), "extra exists on target before drop")

	// 1) DROP TABLE of a migrated table replicates: extra disappears on the target.
	_, err = sc.Query(ctx, "DROP TABLE public.extra")
	require.NoError(t, err)
	require.Eventually(t, func() bool {
		return columnsOf(t, ctx, tc, "public", "extra") == ""
	}, 30*time.Second, 500*time.Millisecond, "DROP TABLE of a migrated table must replicate")

	// 2) DROP TABLE of an unrelated (non-migrated) table must NOT reach the target
	// (it never existed there). Push a migrated-table write AFTER the drop and wait
	// for it to land, so the unrelated drop has had at least as long to (wrongly)
	// replicate — then assert orders still streams (the subscription never stalled).
	_, err = sc.Query(ctx, "DROP TABLE public.unrelated")
	require.NoError(t, err)
	_, err = sc.Query(ctx, "INSERT INTO public.orders (v) VALUES ('after-drop')")
	require.NoError(t, err)
	require.Eventually(t, func() bool {
		n, ok := countRows(t, ctx, tc, "public.orders")
		return ok && n == 4
	}, 30*time.Second, 500*time.Millisecond, "streaming must continue after DROP TABLE")
}
