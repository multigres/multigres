...
// TestActivateDeactivateMigration drives the symmetric switch via the operator
// verbs: after an IMPORT catches up, activate (EXPORT) and prove a write on the
// Multigres target replicates out to the old database; then deactivate (IMPORT)
// and prove a write on the old database replicates back in; then prove
// deactivating an already-importing migration is rejected (self-guarding).
func TestActivateDeactivateMigration(t *testing.T) {
	if testing.Short() {
