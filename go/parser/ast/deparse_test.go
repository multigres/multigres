...
func TestExpressionDeparsing(t *testing.T) {
	tests := []struct {
		...
		{
			name: "and expression",
			node: NewA_Expr(AEXPR_OP, []*String{NewString("AND")},
				NewColumnRef(NewString("a")),
				NewColumnRef(NewString("b")), 0),
			expected: "a AND b",
		},
		{
			name: "or expression",
			node: NewA_Expr(AEXPR_OP, []*String{NewString("OR")},
				NewColumnRef(NewString("x")),
				NewColumnRef(NewString("y")), 0),
			expected: "x OR y",
		},
		{
			name: "not expression",
			node: NewA_Expr(AEXPR_OP, []*String{NewString("NOT")},
				nil,
...
func TestComplexExpressionDeparsing(t *testing.T) {
	tests := []struct {
		...
		// Complex nested expressions
		{
			...
			node: NewA_Expr(AEXPR_OP, []*String{NewString("+")},
				NewColumnRef(NewString("a")),
				NewA_Expr(AEXPR_OP, []*String{NewString("*")},
					NewColumnRef(NewString("b")),
