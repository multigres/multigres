...
func TestRangeVar(t *testing.T) {
	t.Run("SimpleTable", func(t *testing.T) {
		rv := NewRangeVar("users", nil, nil)
		
		require.NotNil(t, rv)
		assert.Equal(t, T_RangeVar, rv.NodeTag())
		assert.Equal(t, "users", rv.RelName)
		assert.Nil(t, rv.SchemaName)
		assert.Nil(t, rv.CatalogName)
	...
	t.Run("SchemaQualified", func(t *testing.T) {
		schema := "public"
		rv := NewRangeVar("users", &schema, nil)
...
func TestResTarget(t *testing.T) {
	...
	t.Run("WithoutName", func(t *testing.T) {
		val := NewString("test")
		rt := NewResTarget(nil, val)
		
		require.NotNil(t, rt)
		assert.Nil(t, rt.Name)
		assert.Equal(t, val, rt.Val)
...
func TestStatementInterfaces(t *testing.T) {
	tests := []struct {
		...
		{"InsertStmt", NewInsertStmt(NewRangeVar("users", nil, nil)), "INSERT"},
		{"UpdateStmt", NewUpdateStmt(NewRangeVar("users", nil, nil)), "UPDATE"},
		{"DeleteStmt", NewDeleteStmt(NewRangeVar("users", nil, nil)), "DELETE"},
		{"CreateStmt", NewCreateStmt(NewRangeVar("users", nil, nil)), "CREATE"},
...
func TestComplexStatementCreation(t *testing.T) {
	t.Run("SELECT with targets and WHERE", func(t *testing.T) {
		...
		nameTarget := NewResTarget(nil, NewColumnRef(NewString("name")))
		ageTarget := NewResTarget(nil, NewColumnRef(NewString("age")))
		
		stmt.TargetList = []*ResTarget{nameTarget, ageTarget}
		
		// Add FROM clause
		fromTable := NewRangeVar("users", nil, nil)
	...
	t.Run("INSERT with columns and VALUES", func(t *testing.T) {
		...
		// Add target columns
		nameCol := NewResTarget(nil, NewString("name"))
		ageCol := NewResTarget(nil, NewString("age"))
	...
	t.Run("UPDATE with SET and WHERE", func(t *testing.T) {
		...
		setTarget := NewResTarget(nil, NewColumnRef(NewString("age")))
...
func TestStatementNodeTraversal(t *testing.T) {
	...
	target := NewResTarget(nil, NewColumnRef(NewString("name")))
	stmt.TargetList = []*ResTarget{target}
	
	// Add a table reference
	table := NewRangeVar("users", nil, nil)
