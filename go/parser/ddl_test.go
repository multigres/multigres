...
func TestDDLParsing(t *testing.T) {
	tests := []struct {
		...
		// Phase 3F: Temporary table creation tests (OptTempTableName)
		{
		...
		// Phase 3F: Identity column option tests (alter_identity_column_option_list)
		{
		...
		// Note: Phase 3F opt_recheck rule is implemented for backward compatibility
		// but is not used in any statements implemented in Phase 3F

		// Key actions tests (foreign key constraints)
		{
		...
		// Phase 3F Round-trip DDL Tests (integrated from TestPhase3F_DDL_RoundTrip)
		{
			name: "Phase 3F: CREATE TABLE basic round-trip",
			sql:  "CREATE TABLE users (id int, name varchar(100))",
		},
		{
			name: "Phase 3F: CREATE TABLE IF NOT EXISTS round-trip",
			sql:  "CREATE TABLE IF NOT EXISTS users (id int, name varchar(100))",
		},
		{
			name: "Phase 3F: CREATE TABLE with PRIMARY KEY round-trip",
			sql:  "CREATE TABLE users (id int PRIMARY KEY, name varchar(100))",
		},
		{
			name: "Phase 3F: CREATE TABLE with NOT NULL round-trip",
			sql:  "CREATE TABLE users (id int NOT NULL, name varchar(100) NOT NULL)",
		},
		{
			name: "Phase 3F: CREATE TABLE with DEFAULT values round-trip",
			sql:  "CREATE TABLE users (id int DEFAULT 0, created_at timestamp DEFAULT NOW())",
		},
		{
			name: "Phase 3F: CREATE TABLE with UNIQUE constraint round-trip",
			sql:  "CREATE TABLE users (id int, email varchar(100) UNIQUE)",
		},
		{
			name: "Phase 3F: CREATE TABLE with CHECK constraint round-trip",
			sql:  "CREATE TABLE users (id int, age int CHECK ((age > 0)))",
		},
		{
			name: "Phase 3F: CREATE TABLE with table-level PRIMARY KEY round-trip",
			sql:  "CREATE TABLE users (id int, name varchar(100), PRIMARY KEY (id))",
		},
		{
			name: "Phase 3F: CREATE TABLE with FOREIGN KEY round-trip",
			sql:  "CREATE TABLE orders (id int, user_id int REFERENCES users (id))",
		},
		{
			name: "Phase 3F: CREATE TABLE with multiple constraints round-trip",
			sql:  "CREATE TABLE users (id int PRIMARY KEY, email varchar(100) UNIQUE NOT NULL, age int CHECK ((age > 0)))",
		},

		// Phase 3F CREATE INDEX Tests
		{
			name: "Phase 3F: CREATE INDEX basic round-trip",
			sql:  "CREATE INDEX idx_users_name ON users (name)",
		},
		{
			name: "Phase 3F: CREATE UNIQUE INDEX round-trip",
			sql:  "CREATE UNIQUE INDEX idx_users_email ON users (email)",
		},
		{
			name: "Phase 3F: CREATE INDEX with multiple columns round-trip",
			sql:  "CREATE INDEX idx_users_name_email ON users (name, email)",
		},
		{
			name: "Phase 3F: CREATE INDEX CONCURRENTLY round-trip",
			sql:  "CREATE INDEX CONCURRENTLY idx_users_name ON users (name)",
		},
		{
			name: "Phase 3F: CREATE INDEX IF NOT EXISTS round-trip",
			sql:  "CREATE INDEX IF NOT EXISTS idx_users_name ON users (name)",
		},
		{
			name: "Phase 3F: CREATE INDEX with WHERE clause round-trip",
			sql:  "CREATE INDEX idx_active_users ON users (name) WHERE (active = TRUE)",
		},

		// Phase 3F ALTER TABLE Tests
		{
			name: "Phase 3F: ALTER TABLE ADD COLUMN round-trip",
			sql:  "ALTER TABLE users ADD COLUMN age int",
		},
		{
			name: "Phase 3F: ALTER TABLE DROP COLUMN round-trip",
			sql:  "ALTER TABLE users DROP COLUMN age",
		},
		{
			name: "Phase 3F: ALTER TABLE ADD CONSTRAINT round-trip",
			sql:  "ALTER TABLE users ADD CONSTRAINT users_email_unique UNIQUE (email)",
		},
		{
			name: "Phase 3F: ALTER TABLE DROP CONSTRAINT round-trip",
			sql:  "ALTER TABLE users DROP CONSTRAINT users_email_unique",
		},
		{
			name: "Phase 3F: ALTER TABLE IF EXISTS round-trip",
			sql:  "ALTER TABLE IF EXISTS users ADD COLUMN age int",
		},

		// Phase 3F DROP Statement Tests
		{
			name: "Phase 3F: DROP TABLE basic round-trip",
			sql:  "DROP TABLE users",
		},
		{
			name: "Phase 3F: DROP TABLE IF EXISTS round-trip",
			sql:  "DROP TABLE IF EXISTS users",
		},
		{
			name: "Phase 3F: DROP TABLE CASCADE round-trip",
			sql:  "DROP TABLE users CASCADE",
		},
		{
			name: "Phase 3F: DROP TABLE RESTRICT round-trip",
			sql:  "DROP TABLE users RESTRICT",
		},
		{
			name: "Phase 3F: DROP INDEX basic round-trip",
			sql:  "DROP INDEX idx_users_name",
		},
		{
			name: "Phase 3F: DROP INDEX IF EXISTS round-trip",
			sql:  "DROP INDEX IF EXISTS idx_users_name",
		},
		{
			name: "Phase 3F: DROP INDEX CONCURRENTLY round-trip",
			sql:  "DROP INDEX CONCURRENTLY idx_users_name",
		},
	}
