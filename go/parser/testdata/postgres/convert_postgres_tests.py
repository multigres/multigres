...
def handle_parameterized_queries(stmt: str) -> str:
    ...
    if stmt.strip().startswith('\\if') or stmt.strip().startswith('\\set') or stmt.strip().startswith('\\echo'):
        return None
    ...
    def is_time_pattern(text, pos):
        ...
        if before_digits and after_digits:
            if len(before_digits) <= 4 and len(after_digits) <= 2:
                ...
                variable_name = re.match(r':\w+', text[pos:]).group()[1:]  # Remove the colon
    ...
    for var in variables:
        ...
        if var_lower in var_replacements:
            processed_stmt = processed_stmt.replace(var, str(var_replacements[var_lower]))
        else:
            ...
            elif 'file' in var_lower and not ('filenode' in var_lower or 'oid' in var_lower):
...
def main():
    ...
    sql_dir = script_dir.parent / "postgres-tests"  # Go up one level to find postgres-tests
