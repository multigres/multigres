...
# Release tag injected into binaries via the Makefile's -ldflags.
# COMMIT and DATE are only consumed by the OCI image labels below, the
# binaries themselves pick up commit SHA and commit time from
# runtime BuildInfo.
ARG VERSION=dev
ARG COMMIT=unknown
ARG DATE=unknown
