...
# Usage: safe_download <url> <output_file>
# Downloads a file from URL and saves it to output_file
# Exits with code 1 and a clear error message if the download fails
safe_download() {
  local url="$1"
  local output_file="$2"

  if [ -z "$url" ] || [ -z "$output_file" ]; then
    echo "❌ Error: safe_download requires both URL and output file" >&2
    echo "Usage: safe_download <url> <output_file>" >&2
    return 1
  fi
