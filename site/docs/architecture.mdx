...
This deployment style will allow for a large number of databases to be deployed under a single logical Postgres cluster.
...
Multigres can be configured to add replicas as standbys. In this setup, we introduce the `MultiOrch` component that will handle the orchestration of these replicas, ensuring they are kept in sync with the primary database.
...
In a multi-cell deployment, the Topo Server will be splittable into multiple instances. It will be recommended that the Global Topo Server be deployed with nodes in multiple regions. For every cell, a cell-specific topo server will be deployable. This will be designed in such a way that components within a cell will mostly depend on the cell-specific topo server.
...
The purpose of this design will be to ensure that a cell that is partitioned from the rest of the system can continue to operate independently for as long as the data is not stale.
...
You will also be able to request MultiOrch to prefer appointing a primary within the same cell to avoid unnecessary churn.
...
In the previous examples where the databases will be unsharded, all the tables will be stored on a single Postgres database. In this situation, there will be a one-to-one mapping between a Multigres database and the Postgres instance.
...
Multigres will use a hexadecimal range based sharding scheme. A shard will be delimited by two values: the start and end of the range. For example, a shard might be defined as `tg1:0-8`. The beginning of the range will be inclusive, while the end will be exclusive. This means that the shard includes keys from `0` up to but not including `8`. The values can contain any number of bits, with the most significant bit being on the left.
...
Multigres will assign a value to each row, known as the `RowKey`. This value will decide which shard the row belongs to. For example, in the case of two shards `0-8` and `8-inf`, a RowKey of `0x01` will be placed in the `0-8` shard, while a RowKey of `0xFF` will be placed in the `8-inf` shard.

Multigres will assign a `RowKey` value to each row based on the type of sharding index assigned to the table column. This functionality will be explained in a different document.
