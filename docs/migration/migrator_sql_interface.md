...
4. **Connection durability.** Named connections are currently held only in the gateway's process memory (`InMemoryConnectionStore`), so they are lost on gateway restart and are not shared across gateway instances. This is asymmetric with migrations: a migration record is persisted in `multigres.migration` on the target primary and physically replicated, so it survives a Postgres primary failover, whereas a connection is not stored on the primary at all. A connection therefore survives a database failover (it is independent of the primary) but not a gateway restart, and an already-created migration keeps working after a restart (its `source_dsn` was resolved and stored at create time) even though the named connection it was created from is gone. Should connections be persisted, and where? The gateway code anticipates a topo-backed store (`// a topo-backed store can replace it later`), which keeps them gateway-scoped — the natural home, since a source connection is not tied to any one target shard's primary the way a migration record is, rather than a per-shard table on the primary.
